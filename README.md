...
$$
无论是引入2之后的\{ 2k + 1 | k \in \mathbb{Z} \}特性还是引入3之后的\{ (2 \cdot 3 \ldots p \cdot m \pm 1) | m \in \mathbb{Z} \}，将会永远传递下去，\\
依次类推，所有新引入素数p产生的特性都会传递。
$$
...
可以，在明确素数特性100%传递的基础之上，假如在达到一个非常大的素数 $p$ 之后之后，不再出现新的素数，那么我们可以再构造一个这样的数：
